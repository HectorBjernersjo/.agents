...
Start by running git status. If there are any uncommitted changes (except for `llm_stuff/prs/<current-branch>/REVIEW_CONTEXT.md`), tell the user to commit them before you proceed, so `git diff origin/master...` works properly.
...
When going through the comments, look in `docs/prs/<current-branch>/REVIEW_CONTEXT.md` (if it exists) to not make the same misstakes as the previous reviewer.
